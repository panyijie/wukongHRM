...
class UserAction extends Action {
	...
	*/
    public function _initialize(){
		$actions = array(
			'users'=>array('logout','getfromuserindex','getuserindex','getfromuserrindex','getuserrindex','getsubuserdialog','getsubusercbdialog','getdepartmentuser','getpositionuser'),
    ...
    public function login(){
        if (session('?name')){
            //判断名称为name的session是否已经进行设置
			$this->redirect('core/index/index',array(), 0, '');

		}elseif($this->isPost()){
            $name     = $this->_post('name','trim','');//三个参数分别表示着：参数的名称，过滤的方式（trim()表示去除key前后的空白字符），默认值）
            $password = $this->_post('password','trim','');
			if(!$name || !$password){
				alert('error', '请正确输入用户名和密码！',U('core/user/login'));
                //error说明发生了错误，（应该是定向到某个方法触发js的弹出效果或者是样式，中间为内容，最后U方法进行重定向到user/login而不需要用户进行点击关闭）
			}else{
                $d_user = D('User');
	...
	*/	
    ...
    /* 2015-07-22
     * 扩展多选内容，新增from_user_id的多选（本质上和上面的多选是一样的）
     * */
    public function getFromUserIndex(){
        $d_user = D('User');
        $department_list = D('Structure')->getDepartmentList(0,'--',1);
        $this->assign('department_list',$department_list);
        $userlist = $d_user->getUserList();
    ...
    /* 2015-07-22
     * 扩展单选内容，新增from_user_id的多选（本质上和上面的多选是一样的）
     * */
    public function getFromUserRIndex(){
        $d_user = D('User');
        $department_list = D('Structure')->getDepartmentList(0,'--',1);
        $this->assign('department_list',$department_list);
        $userlist = $d_user->getUserList();
