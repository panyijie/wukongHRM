...
class AppraisalManagerModel extends Model{
    ...
    public function getAppraisalManager($p, $where){
        ...
        foreach($appraisal_manager_list as $key=>$val){
            ...
            //状态
            if(1 == $val['status']){
                $appraisal_manager_list[$key]['status_name'] = '被考核者评分中';
            }elseif(2 == $val['status']){
                $appraisal_manager_list[$key]['status_name'] = '已汇总';
            }elseif(4 == $val['status']){
                $appraisal_manager_list[$key]['status_name'] = '待汇总';
            }elseif(5 == $val['status']){
                $appraisal_manager_list[$key]['status_name'] = '待被考核者确认';
            }elseif(6 == $val['status']){
                $appraisal_manager_list[$key]['status_name'] = '待主管确认';
            }else{
                $appraisal_manager_list[$key]['status_name'] = '主管评分中';
            }
            //考核进度
    ...
    public function getAllAppraisalManager($where){
        ...
        foreach($appraisal_manager_list as $key=>$val){
            ...
            $appraisal_manager_list[$key]['session_user_name'] = $user['name'];
    ...
    public function getAppraisalManagerById($appraisal_manager_id){
        ...
        }elseif(5 == $appraisal_manager['status']){
            $appraisal_manager['status_name'] = '待被考核者确认';
        }elseif(6 == $appraisal_manager['status']){
            $appraisal_manager['status_name'] = '待主管确认';
        }else{
    ...
    /**
     * 根据id对考核模版进行查询（提前规定名字是不一样的这样可以方便进行查询）
     **/
