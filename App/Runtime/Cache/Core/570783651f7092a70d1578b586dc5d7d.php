<?php if (!defined('THINK_PATH')) exit();?><div class="modal-dialog">
	<div class="modal-content">
		...
		<div class="modal-body">
			...
			<table class="table" id="users_list" style="margin-bottom:0px;">
				<thead>
					<tr>
						<th>&nbsp;</th>
						<th>用户名</th>
				...
				<tbody>
					<?php if(is_array($userlist)): $i = 0; $__LIST__ = $userlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						<td><input type="radio" name="r_user_id" value="<?php echo ($vo["user_id"]); ?>"></td>
...
<script type="text/javascript">
	$(function(){
		$('#check_ed').click(function(){
			var str = $("input[name='r_user_id']:checked").val();
			var name = $("input[name='r_user_id']:checked").parent().next().html();
			$('#from_name').val(name);
			$('#from_user_id').val(str);
			$('#alert').modal('hide');
		});
			...
			$.get('<?php echo U("core/user/getdepartmentuser","id=");?>'+department_id,function(user_list){
				if(user_list){
					...
					$.each(user_list, function(k, v){
						var sex = v.sex == 1 ? '男':'女';
						user_str += '<tr><td><input type="radio" value="'+v.user_id+'" name="r_user_id"></td><td>'+v.name+'</td><td>'+sex+'</td><td>'+v.department_name+'-'+v.position_name+'</td><td>'+v.telephone+'</td><td>'+v.email+'</td></tr>';
					});
