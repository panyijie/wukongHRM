...
<div class="body-right">
	<div class="row-table">
		<div class="row-table-title">合同类型</div>
		<div class="row-table-body">
			<form class="form-horizontal " enctype="multipart/form-data" action="<?php echo U('core/setting/contractType');?>" method="post">
				<p class="form-title">合同类型设置&nbsp;&nbsp;<a href="javascript:history.go(-1);">返回</a></p>
				<div id="type_box">
					<?php if(is_array($contracttype)): $i = 0; $__LIST__ = $contracttype;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="form-group">
						...
						<?php if($key > 3): ?><input type="button" class="btn btn-xs deletecontractType" value="删除" /><?php endif; ?>
					</div><?php endforeach; endif; else: echo "" ;endif; ?>
				</div>
				<div class="form-group">
					<label class="col-sm-2 control-label"></label>
					<div class="col-sm-3">
						<input type="button" class="btn btn-primary"  id="addcontractType" value="添加" />&nbsp;&nbsp;
...
$(function(){
	$('#addcontractType').click(function(){
		...
		if(num > 3){
			str+='<input type="button" class="btn btn-xs deletecontractType" value="[ 删除 ]" />';
		}
		str+='</div>';
		$('#type_box').append(str);
	});
	$('#type_box').on('click','.deletecontractType',function(){
		$(this).parent().remove();
	});
})
</script>
