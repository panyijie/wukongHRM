...
<div class="body-right">
	<div class="row-table">
		<div class="row-table-title">添加加班</div>
		<div class="row-table-body">
			<form class="form-horizontal " action="<?php echo U('hrm/overtime/add');?>" method="post">
				<p class="form-title">添加加班&nbsp;&nbsp;<a href="javascript:history.go(-1);">返回</a></p>
				<div class="form-group">
					<label for="name" class="col-sm-2 control-label">加班人</label>
					<div class="col-sm-3">
						<input type="hidden" name="user_id" id="dialog_user_id" value="" />
						<input class="form-control" type="text" name="user_name" id="dialog_user_name"/>
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-3">
						<select class="form-control" name="overtime_category_id">
							<?php if(is_array($category)): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["overtime_category_id"]); ?>"><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
						</select>
					</div>
					<label for="name" class="col-sm-2 control-label">结算方式</label>
					<div class="col-sm-3">
						<input type="radio" name="type" value="0" checked="checked">&nbsp;计费加班&nbsp;&nbsp;
						<input type="radio" name="type" value="1">&nbsp;调休&nbsp;&nbsp;
					</div>
				</div>
				<div class="form-group">
