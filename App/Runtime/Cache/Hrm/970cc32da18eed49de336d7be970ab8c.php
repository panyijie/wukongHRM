	...
	<div class="row-table">
		...
		<div class="row-table-body">
			<p class="form-title">
				...
				部门岗位图 | 
				<a href="<?php echo U('hrm/structure/position');?>" >上下级关系图</a>
			</p>
			<?php echo ($tree_code); ?>
		</div>
	</div>
...
<script type="text/javascript">
	$(function(){
		...
		$(".department_delete").click(function(){
			if(confirm("确定删除该部门吗?")){
				id = $(this).attr('rel');
				window.location="<?php echo U('hrm/structure/deleteDepartment','id=');?>"+id;
			}
		});
		...
		$('.department_edit').click(function(){
			...
			$('#alert').modal({
			show:true,
			remote:'<?php echo U("hrm/structure/editDepartment","id=");?>'+department_id
			});
		});
