...
<div class="body-right">
	<div class="row-table">
		<div class="row-table-title">调休管理</div>
		<div class="row-table-body">
			<p class="form-title">
				调休管理&nbsp;&nbsp;
				<a href="<?php echo U('hrm/lieu/add');?>" class="pull-right btn btn-primary btn-xs" >添加</a>
				<form id="search_form" action="" method="get">
					<input type="hidden" name="g" value="hrm"/>
					<input type="hidden" name="m" value="lieu"/>
			...
			<?php if(empty($lieulist)): ?><div>---暂无数据---</div>
			<?php else: ?>
			<form class="form-horizontal " id="form1"  method="post">
				<table class="table" style="margin-bottom:0px;">
					<tbody>
						<tr>
							<th><input type="checkbox" name="lieu_id[]" id="check_all"/></th>
							<th>调休人</th>
							<th>填写人</th>
							<th>状态</th>
							<th>调休类型</th>
						...
						<?php if(is_array($lieulist)): $i = 0; $__LIST__ = $lieulist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							<td><input type="checkbox" name="id[]" class="check_list" value="<?php echo ($vo["lieu_id"]); ?>" /></td>
							...
							<td><?php echo ($vo["lieu_days"]); ?></td>
							<td><?php echo ($vo["lieu_hours"]); ?></td>
							<td><?php echo (date('Y-m-d H:i:s',$vo["create_time"])); ?></td>
							<td>
								<a href="<?php echo U('hrm/lieu/view','id='.$vo['lieu_id']);?>">查看</a>&nbsp;|&nbsp;
								<a href="<?php echo U('hrm/lieu/edit','id='.$vo['lieu_id']);?>">编辑</a>&nbsp;|&nbsp;
								<a href="<?php echo U('hrm/lieu/delete','id='.$vo['lieu_id']);?>">删除</a>
							</td>
...
<script type="text/javascript">
	...
	$("#delete").click(function(){
		if(confirm('确定要删除吗？')){
			$("#form1").attr("action","<?php echo U('hrm/lieu/delete');?>");
			$("#form1").submit();
		}
	});
	...
	$("#select_status").change(function(){
		if(confirm('确定要批量审核吗？')){
			$("#form1").attr("action","<?php echo U('hrm/lieu/auditing');?>");
			$("#form1").submit();
		}
	});
	...
	$(function(){
		...
		$('.btn.search_btn').dblclick(function () {
			clearTimeout(TimeFn);
			window.location.href="<?php echo U('hrm/lieu/index');?>";
		})
	});
</script>
